...
.dokan-warning.phone-warning a {
    ...
    echo '<span class="dokan-label dokan-label-warning dokan-product-status-label">'.$product->get_stock_quantity().' In Stock</span>';
    elseif($product->managing_stock() && !$product->is_in_stock() )
 echo '<span class="dokan-label dokan-label-danger dokan-product-status-label">Out of Stock</span>';
                    ?>
					 <?php if ( $_visibility == 'hidden' ) { ?>
                        <span class=" dokan-label dokan-label-default dokan-product-hidden-label"><i class="fa fa-eye-slash"></i> <?php esc_html_e( 'Hidden', 'dokan-lite' ); ?></span>
                    <?php } ?>
					</div>
                    <?php if ( $post->post_status == 'publish' ) { ?>
                        <span class="dokan-right">
							...
							<?php endif ?>
                            <a class="dokan-btn dokan-btn-theme " href="<?php echo esc_url( get_permalink( $post->ID ) ); ?>" target="_blank"><?php esc_html_e( 'View Product', 'dokan-lite' ); ?></a>
                        </span>
                    <?php } ?>
					<?php 
					$shipping_classes = get_the_terms( $post->ID, 'product_shipping_class' );
					if(!empty($shipping_classes)){
						$shipping_class = current($shipping_classes)->name; ?>
						<span class="dokan-label dokan-label-default dokan-shipping-label"><i class="fas fa-coins"></i> <?php esc_html_e( $shipping_class , 'dokan-lite' ); ?></span>
						<style>
							span.dokan-label.dokan-label-default.dokan-shipping-label {
								font-size: 11px;
								margin-right: 20px;
			...
			<?php 
				...
				$class_key = 'class_cost_' . current($shipping_classes)->term_id;
				...
				if(!empty( $shipping_methods)){
					if(!empty($shipping_classes) && current($shipping_classes)->term_id != 515){
						foreach( $shipping_methods as $method){
							if($method['id'] == 'flat_rate' && $method['enabled'] == 'yes'){
								if(!isset($method['settings'][$class_key]) or $method['settings'][$class_key] == ''){
									echo '<p class="dokan-warning phone-warning">You have set shipping class of this product to "'. current($shipping_classes)->name .'" but price of this class is not set in one of your shipping method. <a href="/dashboard/settings/shipping/">Click here to set</a>.</p>';
									break;
								}
							}
						}
					}elseif(current($shipping_classes)->term_id == '515'){
					
					}else{
						echo '<p class="dokan-warning phone-warning">No shipping class assigned to this product. Please assign shipping class.</p>';
					}
					
				}else{
					echo '<p class="dokan-warning phone-warning">You have not setup any shipping methods. <a href="/dashboard/settings/shipping/">Click here to setup</a>.</p>';
				}
...
<!--                             <a href="<?php echo esc_url( get_permalink( $post_id ) ); ?>" target="_blank"><?php esc_html_e( 'View Product &rarr;', 'dokan-lite' ); ?></a> -->
						<a class="add-another-product-btn" href="/dashboard/new-product/"><i class="fa fa-plus"></i><?php esc_html_e( ' Add another product', 'dokan-lite' ); ?><a/>
                        <?php } ?>
                    </div>
                <?php } ?>
...
<?php if ( !empty( $_GET['new'] ) && empty($_GET['message'] == 'success' ) ) { ?>
                    <div class="dokan-message">
                        <button type="button" class="dokan-close" data-dismiss="alert">&times;</button>
                        <?php echo sprintf( '<b>Nice</b> Your Plant was saved. Don\'t forget to update the attributes and shipping below. &#128521', max( ( isset( $post_id ) ? $post_id : 0 ), (int)$_GET['new'] ) ) ?>
                    ...
                    if ( dokan_is_seller_enabled( get_current_user_id() ) ) { ?>
                        <form class="dokan-product-edit-form" role="form" method="post">
                            ...
                            <div class="dokan-form-top-area">
                                ...
                                <div class="content-half-part dokan-product-meta">
                                    ...
                                    <div class="dokan-form-group">
                                        ...
                                        <label for="post_title" class="form-label"><?php esc_html_e( 'Title', 'dokan-lite' ); ?><span style="font-size:12px;"> (Product title length should not be greater than 65 characters.)</span></label>
										 <input class="dokan-form-control" maxlength='65' name="post_title" id="post_title" type="text" placeholder="<?php esc_attr_e( 'Product name..', 'dokan-lite' ); ?>" value="<?php echo $post_title; ?>">
                                        <?php // dokan_post_input_box( $post_id, 'post_title', array( 'maxlength'=>'65', 'placeholder' => __( 'Product name..', 'dokan-lite' ), 'value' => $post_title ) ); ?>
                                ...
                                <label for="post_excerpt" class="form-label"><?php esc_html_e( 'Short Description', 'dokan-lite' ); ?><span style="font-size:12px;"> (Maximum allowed characters 300.)</span></label>
								<textarea rows="3" class="dokan-form-control" style="max-height:unset !important;" name="post_excerpt" maxlength="300"><?php echo $post_excerpt;?></textarea>
                                <?php //wp_editor( $post_excerpt , 'post_excerpt', apply_filters( 'dokan_product_short_description', array( 'editor_height' => 1, 'quicktags' => false, 'media_buttons' => false, 'teeny' => true, 'editor_class' => 'post_excerpt' ) ) ); ?>
                            </div>
                                ...
                                <div class="content-half-part featured-image">
                                    ...
                                    <div class="dokan-feat-image-upload dokan-new-product-featured-img">
                                        ...
                                        <div class="instruction-inside<?php echo esc_attr( $instruction_class ); ?>">
                                            ...
                                            <a href="#" class="dokan-feat-image-btn dokan-btn"><i class="fa fa-cloud-upload"></i><?php esc_html_e( 'Upload a product cover image', 'dokan-lite' ); ?></a>
                                        </div>

                                        <div class="image-wrap<?php echo esc_attr( $wrap_class ); ?>">
                                            <a class="close dokan-remove-feat-image"><p class="delete-it">&times;</p></a>
                            ...
                            <div class="dokan-product-description">
                                <label for="post_content" class="form-label"><?php esc_html_e( 'Full Description', 'dokan-lite' ); ?></label>
                            ...
                            <input type="submit" name="dokan_update_product" class="dokan-btn dokan-btn-theme  dokan-right" value="<?php esc_attr_e( 'Save Product', 'dokan-lite' ); ?>"/>
